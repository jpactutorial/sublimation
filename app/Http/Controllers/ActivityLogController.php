...
END AS parent_type_title,
    ...
    "
        ...
        if ($type && $type_id) {
            $activity_log_query->where(function ($query) use ($type, $type_id) {
        ...
        $activity_log = $activity_log_query->orderBy($sort, $order)
            ...
            ->through(function ($activity_log) use ($canDelete) {
                ...
                $actions = '';
                if ($canDelete) {
                    $actions = '<button title="' . get_label('delete', 'Delete') . '" type="button" class="btn delete" data-id="' . $activity_log->id . '" data-type="activity-log" data-table="activity_log_table">' .
