    ...
    public function list()
    {
        ...
        $estimates_invoices = $estimates_invoices->orderBy($sort, $order)
            ...
            ->through(function ($estimates_invoice) use ($canEdit, $canDelete, $canCreate) {
                ...
                if ($canEdit) {
                    $actions .= '<a href="/estimates-invoices/edit/' . $estimates_invoice->id . '" title="' . get_label('update', 'Update') . '"><i class="bx bx-edit mx-1"></i></a>';
                }
                ...
                $actions .= '<a href="/estimates-invoices/pdf/' . $estimates_invoice->id . '" title="PDF" target="_blank">' .
