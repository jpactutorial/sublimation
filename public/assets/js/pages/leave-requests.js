...
$('#lr_status_filter,#lr_user_filter,#lr_action_by_filter,#lr_type_filter').on('change', function (e) {
    e.preventDefault();
...
function actionsFormatter(value, row, index) {
    ...
    if (isAdmin || row.action_by === null) {
        actions += '<a href="javascript:void(0);" class="edit-leave-request" data-bs-toggle="modal" data-bs-target="#edit_leave_request_modal" data-id=' + row.id + ' title=' + label_update + ' class="card-link"><i class="bx bx-edit mx-1"></i></a>';
    }

    if (isAdminOrLe) {
        actions += '<button title=' + label_delete + ' type="button" class="btn delete" data-id=' + row.id + ' data-type="leave-requests" data-table="lr_table">' +
